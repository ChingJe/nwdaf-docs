...
- 但 broader lifecycle cancellation 還沒完全收乾淨，後面仍留在 Priority 2
...
### 6.1 Priority 2 還沒有完全結束

雖然 notifier 主 scheduler ownership 已修，external-client round 也順手補了一些
shutdown / convergence 問題，但 `remediation batches` 目前仍把 Priority 2 判為
partially completed。

仍殘留的本質是：

1. app-owned I/O context 是否已全面收斂
2. background work 是否都從正確的 cancellation tree 取得 context
3. 某些 runtime network path 是否還有 `context.Background()` 型態的殘留

換句話說，現在不是「完全沒人管 lifecycle」，而是：

> 大方向對了，但還沒做到整個 repo 的所有長生命週期與 I/O path 都完全一致。

### 6.2 SMF raw HTTP exception 仍存在
...
### 6.3 covered scope 與 full free5GC parity 不是同一件事

Priority 4、5、8 都是針對 current intended standalone-NWDAF alignment level
完成，而不是宣告：
...
目前 `api_mlmodelprovision.go` 已往 generated model 靠攏，但這只是部分完成。
...
1. 全 repo 更嚴格的 lifecycle 收斂
2. 對外 signaling 與 observability 的成熟度
3. standardized model / local extension 的治理規則
4. runtime config scope 重切
5. NWDAF 最終 free5GC integration level 的策略選擇
6. repository 邊界的最後整理

因此現在的狀態比較適合描述成：

> NWDAF 已經從「表面像 free5GC-style NF，但實際邊界不夠一致」
> 走到「共享 runtime boundary、測試 seam、consumer ownership、config contract
> 已大幅收斂」，但還沒有走到 full integration-level closure。
