...
Status: High-level architecture confirmed; phase-level implementation plans pending
...
12. 保留現有 Go policy/training production path，Phase 1 不切換 owner
...
5. 執行 Go tests、race、lint、build 與 Python unit/API tests
...
section 4 定案。以下只剩會影響 phase 實作 contract 或 deployment 的 exact choices，不重新打開
已確認的 architecture boundary。

以下項目尚未定案，必須在對應 phase plan 開始前由使用者確認，不得由實作者自行選擇：

1. `PyMTLF/` exact package name、Python runtime framework 與 port
2. Go/MTLF-backend internal API 的 exact HTTP paths 與 authentication；path/metadata 不得包含
   Python implementation naming
3. dataset chunk acknowledgement、retry 與同一 live attempt 內的 resume protocol；aborted
   attempt 與 process restart 不跨 attempt resume
4. common `TrainingObservation` exact schema 與 schema version policy
5. MongoDB database/collection naming、retention duration、index set 與 migration policy
6. ADRF request timeout/retry/watchdog 的 exact 數值，以及 valid empty result 是否可由明確
   `fallbackOnEmpty` config 覆寫預設的 no-fallback behavior
7. partial dataset 是否可訓練，以及最低 completeness threshold
8. local training algorithm、fixture dataset 與 candidate evaluation criteria
9. bundle v1 manifest exact schema、archive size limit、supported runtime versions 與
    compatibility validation
10. PyMTLF private model download endpoint 的 exact route、authorization、base URL、URL lifetime、
    integrity header、Go allowlist 與 artifact cleanup policy
11. trusted-local `model.py`/pickle bundle 何時及如何升級成可接受 external NWDAF artifact 的
    signed or safer format
12. manual training operation 是否需要，以及其 owner/authorization semantic
13. 現有 `externalMtlf` config/test naming 如何收斂成「另一個含 MTLF 的 NWDAF」語意；
    standard consumer capability 不得與 legacy framework cleanup 一起誤刪
14. full 5GC E2E 所需 ADRF/Mongo/PyAnLF/PyMTLF environment 與驗證時間點
