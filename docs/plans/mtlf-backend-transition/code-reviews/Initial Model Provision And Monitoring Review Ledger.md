...
Status: Open; confirmed defects and required process-level verification remain unresolved
...
|---|---|---|---|---|
| IMR-01 | P1 | Unified sync | Confirmed defect | Rejected sync對所有active domain state為zero-side-effect |
| IMR-02 | P1 | Monitor lifecycle | Confirmed defect | restart後的orphan monitor resource會被清除，且不能再驅動policy |
| IMR-03 | P2 | Model reuse | Confirmed defect | resolver能從active catalog找回真正相容的model，不依賴單一latest pointer |
| IMR-04 | P2 | Monitor semantics | Confirmed defect | per-resource cadence、multiple modelIds、optional context及immReport依contract處理 |
| IMR-05 | P2 | Representation ownership | Confirmed defect | consumer不能注入並回顯provider-owned response fields |
| IMR-06 | P2 | Go standard boundary | Confirmed defect | error/redirect依operation allowlist處理，private backend URI不外洩 |
| IMR-V01 | Blocking gate | Cross-process integration | Missing required evidence | 實際三process initial model與monitor round-trip通過 |
