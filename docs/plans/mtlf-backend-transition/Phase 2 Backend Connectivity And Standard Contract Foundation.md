...
Status: Implemented locally; deployment integration validation pending
...
放進factory config。availability monitor自己以mutex保護snapshot；processor與coordinator只透過窄介面讀取狀態。
...
每個configured backend使用獨立availability monitor：
...
disabled backend不建立availability monitor，以`DISABLED`作為config-derived狀態回報；`DISABLED`不是worker state。

### 6.2 Snapshot

availability monitor至少提供不可變snapshot：
...
- `internal/backend/availability_monitor.go`
  - 新增backend-neutral availability monitor、state snapshot、backoff與availability error
...
- `internal/anlf/coordinator/`
  ...
  - unusable時不發live request；transport/5xx failure回報availability monitor
...
- `pkg/service/init.go`
  - `NewApp`只construct clients/availability monitors，不做network call
  - `startRuntime`啟動兩個app-owned availability workers
...
### Step 2: Build Backend-neutral Availability Monitor

1. 以fake probe建立state/backoff/cancellation tests
2. 完成availability monitor與snapshot
3. 驗證兩個availability monitor可獨立運作且無global state
4. 跑Go race tests
...
3. MTLF availability monitor在每次successful readiness後重新計算sources並handshake
...
3. listener startup後啟動availability workers
...
17. Mongo ping timeout或app cancellation不會阻塞availability worker shutdown
...
| PyMTLF down、PyAnLF up | existing AnLF analytics不因MTLF availability monitor停止 |
...
Phase 2的local implementation已完成，實際變更如下：

- `NWDAF/`
  - 新增backend-neutral availability monitor，包含independent worker、cached snapshot、bounded backoff、
    jitter、handshake state、immediate retry wake-up與app-owned cancellation
  - AnLF與MTLF client均加入bounded readiness transport；MTLF client另實作最小storage-mode handshake
  - `NwdafApp`在listener啟動後管理兩個availability monitor，並在每次MTLF handshake前重新計算ADRF/Mongo source
    inventory；Mongo使用2秒bounded live ping
  - current AnLF operation改讀cached gate；transport與5xx會使connection unavailable，4xx不會誤判
    process down
  - Events Subscription在backend configured但unusable時回不洩漏internal identity的503
    `ProblemDetails`
  - NRF profile依AnLF config決定是否advertise Events Subscription；runtime health transition不造成profile
    churn
  - AnLF config加入5秒request timeout與HTTP origin normalization
- `PyAnLF/`
  - 加入`/health/live`與`/health/ready`
  - readiness綁定manager initialization、request acceptance與shutdown lifecycle，不要求active model
- `PyMTLF/`
  - 加入`POST /internal/v1/data-source-selection`，固定`adrf`、`mongodb`、`dual`三種mode與409 mismatch
    semantics
  - readiness保留artifact repository probe，storage selection state在每次process lifespan重新初始化
  - 完整移除Phase 1 SQLite generation journal、reconciliation engine、專用config、models、tests與state文件

本phase沒有修改current UPF writer、Daisy training path、ADRF fetch procedure或standard feature payload
ownership；這些仍由Phase 3至Phase 6接管。

### 14.2 Verification Completed

已完成：

- `PyAnLF/`: `145 passed, 1 skipped`; Ruff通過
- `PyMTLF/`: `42 passed`; Ruff通過
- `NWDAF/`: Phase 2 focused Go tests通過
- `NWDAF/`: backend、AnLF、MTLF client與service race tests通過
- `NWDAF/`: `make test`、`make build`、`make lint`通過，lint為0 issues
- real PyAnLF與PyMTLF process-level contract smoke tests通過
- backend停止時Go live contract test可偵測connection failure；backend重啟後同一test可恢復成功
- `git diff --check`通過，且未新增`testdata` JSON contract fixture

### 14.3 Environment Validation Still Pending

以下需要可用的完整deployment environment，尚未宣稱完成：

- NWDAF、real NRF、PyAnLF與PyMTLF共同啟動的完整registration與request-flow驗證
- real ADRF availability與fetch-instruction environment
- real Mongo late-start、disconnect與recovery的cross-process驗證
- deployment TLS與OAuth組合
- negotiated mode在Phase 5接管raw notification writer與direct retrieval後的end-to-end data path

上述pending項目不阻擋Phase 3使用本phase的connection gate，但在deployment acceptance前仍須執行。
...
使用同一backend availability狀態routing ML Model Monitor operations；Release 18 monitor compatibility types在該phase
依實際route建立。
