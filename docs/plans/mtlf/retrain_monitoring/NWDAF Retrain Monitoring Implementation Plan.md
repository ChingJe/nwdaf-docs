...
| Log-derived analysis report / observability contract | `internal/anlf` + `internal/mtlf` + `tools/retrain_analysis` | 新 |
...
- 第一版優先考慮 normalized metric（`WAPE / NRMSE`），`MAE` 僅保留為明確 opt-in 選項
- aggregation 只保留：
  - `mean`
  - `percentile`
- `median` 不作為核心獨立選項；若要 median，應以 `percentile=50` 表示
...
**決策結論**
- 新規則抽象成 `decisionWindowSize = M` 與 `requiredHitsInWindow = N`
- 必須滿足 `1 <= N <= M`
- 當 `M = N` 時，可退化成目前 strict consecutive 行為
- degradation path 與 chronic path 可共用同一組 window 參數，後續若需要再拆分
...
| both-zero 保留 | `actual=0 AND pred=0` 的 sample 視為真實觀測，保留在 recent buffer 與 baseline 中；是否 retrain 交由 retrain decision paths + breach policy 決定 |
...
### 4.7 觀測資料與離線分析（log-only）

**目的**：保留足夠的決策與流量時間線資訊，支撐離線分析、threshold tuning 與 retrain 行為驗證。

**定位**
- 觀測來源統一為應用程式 log，不再另外維護 accuracy CSV dump
- 離線分析工具直接讀取 `nwdaf.log` 與 config，產出單一 HTML 報表

**log 需提供的資料**

**(a) Policy / metric log**
- `Accuracy scope ... metrics=... samples=...`
- `Accuracy policy ... current/mean/std/zscore/baselineReady/degradationSignal/chronicSignal/...`

用途：
- 重建各 metric 的時間序列
- 檢視 `baselineReady`、eligibility、signal、window hit 等決策狀態
- 驗證 cold start 期間「值持續計算、signal 標 `skipped`、但不累積決策 hit」是否符合設計

**(b) Traffic / lifecycle log**
- `latest aggregated slot ... ulVol/dlVol/...`
- `ML inference ... ulVol/dlVol/...`
- retrain lifecycle event：trigger、training start / done、swap done

用途：
- 畫 actual vs predicted UL / DL traffic
- 在同一條時間線對照 retrain 與 signal 的先後關係

**注意事項**
- `Accuracy scope` 與 `Accuracy policy` log 是正式觀測契約的一部分，格式變更需同步更新分析工具
- log-only 設計下，不再維護 `metrics.csv` / `pairs.csv` 的 schema 與相容性
- 若未來 `ML inference` 出現 `steps > 1`，需重新評估 log 是否仍能完整支撐 high-resolution traffic reconstruction

**任務**
- 維持足夠的 policy / metric / traffic / lifecycle log 欄位穩定
- 提供離線報表工具，從 log 重建 metric、signal、traffic 與 retrain timeline
- 報表需明確標示 `skipped` signal 與各 threshold / eligibility 條件
...
**新增：chronic policy config**

| 欄位 | 型別 | 說明 | 預設（暫定） |
|------|------|------|------|
| `chronicPolicy.enabled` | `bool` | 是否啟用 chronic poor-quality path | `false` |
...
- Milestone 1：跑一段時間後離線分析報表（metric 相關係數、scope 分布差異）
...
### Checkpoint B — Per-scope baseline + retrain decision paths + cold start
...
### 6.5 `fixedFloor` 的具體數字（MAE 單位）
先給一個暫定值不卡住實作；跑起來後看分析報表與 log 調整。Checkpoint C 正式定案。

### 6.6 Chronic path 與 breach policy 補充設計紀錄
`chronic poor-quality path`、`M-of-N breach policy`、以及對應 config 是在 Checkpoint B 期間補入的設計擴充；文件進度與落地狀態仍由 workflow 文件維護。
