...
### 4.5.2 Chronic poor-quality path（後續補充設計）
...
### 6.8 Chronic path 與 breach policy 的後續補充
`chronic poor-quality path`、`M-of-N breach policy`、以及對應 config 作為 Checkpoint B 的後續補充設計項目；實際進度與落地狀態由 workflow 文件維護。
