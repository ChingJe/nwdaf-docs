...
1. `internal/anlf/accuracy/monitor.go` 負責 prediction/ground-truth matching、metric calculation
   與 report generation
...
實作完成後，各 package 可用以下方式理解：

| Package | 簡要責任 |
| --- | --- |
| `internal/anlf` | AnLF auxiliary HTTP edge；管理 server、routes、request parsing 與 HTTP status mapping |
| `internal/anlf/contract` | 定義 NWDAF 與 AnLF Backend 交換的 runtime、provision、observation 與 analytics report JSON 資料 |
| `internal/anlf/client` | Go 呼叫 AnLF Backend 的 HTTP transport；負責 URL、JSON、timeout 與 response status handling |
| `internal/anlf/processor` | AnLF callback use-case 入口；接收 analytics report 或 model provision notification，再呼叫窄 coordinator/notifier interface |
| `internal/anlf/coordinator` | 協調 Go subscription、backend runtime、observation bindings、provision callback 與 observation worker；不執行 analytics runtime |
| `internal/anlf/accuracy` | Phase 4 前暫留的 prediction bookkeeping、ground-truth matching、accuracy metrics 與 report generation |
| `internal/sbi/notifier` | 將 backend analytics report 映射成標準 Nnwdaf notification，並處理 external delivery、dedup、in-flight 與 stale state |
| `internal/sbi/processor` | 處理標準 SBI subscription/data collection procedure；只透過窄介面使用 AnLF coordinator |
| `pkg/service` | composition root；建立各 component、注入依賴並直接控制 server 與 AnLF-owned worker lifecycle |

主要資料流為：

```text
external NF subscription
    -> SBI API
    -> SBI processor
    -> AnLF coordinator
    -> AnLF client
...
### 15.2 Transitional Model Correlation

目前 coordinator 仍會把 MTLF model provision callback 解析成 per-subscription runtime apply，
並透過 Go `SharedModelInfo` 支援 retrain fan-out 與 accuracy monitor lifecycle。這是
behavior-preserving refactor 所保留的過渡責任，不是最終 ownership。

後續 phase 應改為把完整 MTLF model provision event 交給 AnLF Backend，由 backend 根據
自身保存的 subscription runtime 與 shared model state 決定哪些 subscriptions 需要
replacement、reuse 或 fallback。等 accuracy/retrain flow 不再依賴 Go model registry 後，
應移除 `SharedModelInfo`、`sharedModelRegistry` 與 coordinator 的 model fan-out。
...
1. `go test ./...`
2. `make lint`，結果為 `0 issues`
3. `make build`
4. `go test -race ./internal/anlf/... ./internal/sbi/... ./internal/context ./internal/mtlf/...`
5. `git diff --check`
6. `go list` dependency inspection，無 import cycle
7. 真實 NWDAF-to-PyAnLF live contract test，涵蓋 runtime apply、binding sync、observation、
   analytics callback、external notification 與 runtime release

`PyAnLF/` production code 不需要因本次 package consolidation 修改。
