...
6. accuracy monitor cadence 與 warmup
...
startup-only policy。Phase 4 後「新 generation 重新 warmup」是後續已確認的新決策，但
warmup prediction discard 原則仍應保留。
...
### BP-14: Warmup Predictions Are Retained

Severity: P1

Current warmup只跳過processing；predictions仍進入pending，warmup結束後可能被拿來matching。

Important distinction：

1. generation replacement後重新warmup是Phase 4已確認的新決策，不是regression
2. 每次warmup期間predictions/counters應被丟棄，這是舊Go invariant

Conclusion: report item was partially correct; generation re-warmup itself is intentional.
...
count、warmup、same/different identity separation與successful/failed generation replacement。
...
analytics，並不建立accuracy prediction record；startup warmup內已建立的records也會在warmup
結束時discard。
...
這符合原本「無參考價值的資料不足prediction不應污染baseline」的設計意圖；與historical code
在罕見backend model output本身回傳confidence-zero時的無條件record差異，視為已核准的語意
明確化，必須加入compatibility/policy test。

### BP-30: Phase 4 Measurement Config Is Incomplete

Severity: P1 plan gap

Plan要求或建議的以下config/validation未完整存在：

1. `check_interval`
2. `metrics_to_record`
3. derived `max_miss_count = ceil(2*SI/checkInterval)+1`
4. explicit late observation/miss policy
5. exact slot parameters validation
...
ownership，改回derived miss-count lifecycle；observation source retention仍保留。
...
| 12 warmup changed | Partially correct | re-warmup intentional；retained warmup predictions是bug |
...
9. warmup完成時discard該generation predictions/counters
...
metric。這主要明確化historical insufficient-history fallback與warmup discard的原設計意圖；
不在本輪增加獨立low-confidence report state。
...
不跨round累積到`min_samples`，以恢復historical report cadence與MTLF threshold單位。Prediction
...
retry/discard時間尺度，不作matching tolerance。Observation source retention不受影響。
...
下一步不應直接依掃描報告逐點patch。應依`Behavioral Parity Remediation Plan.md`執行：

1. 從R0建立historical golden fixtures與initial failure evidence
2. 依R1恢復analytics shaping
3. 依R2恢復accuracy measurement semantics
4. 依R3修正identity/generation/provision
5. 依R4完成scheduler/subscription lifecycle
6. 最後執行R5 verification與status closure
