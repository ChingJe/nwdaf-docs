---
spec: TS 29.505
version: 18.8.0
release: '18'
source_archive: 29505-i80.zip
source_document: 29505-i80.docx
source_archive_sha256: b432c3d6d7a211f0d1eeb72da4364d32ba1d9a3ae51c03df8cab38d2700b777e
source_document_sha256: 46edda85efed924d08a4f02c12acb7970b710feae119f61edd880bb6a2db96a3
clause: 5.4.2.24
title: '5.4.2.24 Type: MessageWaitingData'
content_origin: 3gpp-source
conversion: deterministic-pandoc-structure
---

# 5.4.2.24 Type: MessageWaitingData

Table 5.4.2.24-1: Definition of type MessageWaitingData

| Attribute name | Data type       | P   | Cardinality | Description                                                                                |
|----------------|-----------------|-----|-------------|--------------------------------------------------------------------------------------------|
| mwdList        | array(SmscData) | O   | 1..N        | List of addresses of SM-Service Center entities with SMS wating to be delivered to the UE. |
