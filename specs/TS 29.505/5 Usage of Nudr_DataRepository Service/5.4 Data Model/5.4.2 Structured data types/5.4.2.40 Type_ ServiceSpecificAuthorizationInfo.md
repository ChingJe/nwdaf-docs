---
spec: TS 29.505
version: 18.8.0
release: '18'
source_archive: 29505-i80.zip
source_document: 29505-i80.docx
source_archive_sha256: b432c3d6d7a211f0d1eeb72da4364d32ba1d9a3ae51c03df8cab38d2700b777e
source_document_sha256: 46edda85efed924d08a4f02c12acb7970b710feae119f61edd880bb6a2db96a3
clause: 5.4.2.40
title: '5.4.2.40 Type: ServiceSpecificAuthorizationInfo'
content_origin: 3gpp-source
conversion: deterministic-pandoc-structure
---

# 5.4.2.40 Type: ServiceSpecificAuthorizationInfo

Table 5.4.2.40-1: Definition of type ServiceSpecificAuthorizationInfo

| Attribute name                   | Data type              | P   | Cardinality | Description                                                                                                      |
|----------------------------------|------------------------|-----|-------------|------------------------------------------------------------------------------------------------------------------|
| serviceSpecificAuthorizationList | map(AuthorizationInfo) | M   | 1..N        | A map (list of key-value pairs where authId (see 3GPP TS 29.503 \[6\]) serves as key) of AuthorizationInfo items |
