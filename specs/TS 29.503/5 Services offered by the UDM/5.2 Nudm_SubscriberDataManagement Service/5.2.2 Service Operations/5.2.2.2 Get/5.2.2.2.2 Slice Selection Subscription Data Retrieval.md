...
release: '18'
clause: 5.2.2.2.2
title: 5.2.2.2.2 Slice Selection Subscription Data Retrieval
...
Figure 5.2.2.2.2-1 shows a scenario where the NF service consumer (e.g. AMF) sends a request to the UDM to receive the UE's NSSAI (see also clause 5.15.5.2.1 of TS 23.501 \[2\] and TS 23.502 \[3\] figure 4.2.2.2.3-1 step 3). In this example scenario the NSSAI is retrieved by the AMF, see clause 6.1.3.2.1 for other scenarios that can retrieve the NSSAI. The request contains the UE's identity (/{supi}), the type of the requested information (/nssai) and query parameters (supported-features, plmn-id).
