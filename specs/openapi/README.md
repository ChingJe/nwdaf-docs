# Official OpenAPI attachments

These files are copied byte-for-byte from the supplied Release 18 specification packages. Package versions and attachment-declared point releases are intentionally not normalized.

## Included files

- [TS29122_AsSessionWithQoS.yaml](TS29122_AsSessionWithQoS.yaml) — 3GPP TS 29.122 V18.9.0 T8 reference point for Northbound APIs
- [TS29122_ChargeableParty.yaml](TS29122_ChargeableParty.yaml) — 3GPP TS 29.122 V18.6.0 T8 reference point for Northbound APIs
- [TS29122_CommonData.yaml](TS29122_CommonData.yaml) — 3GPP TS 29.122 V18.6.0 T8 reference point for Northbound APIs
- [TS29122_CpProvisioning.yaml](TS29122_CpProvisioning.yaml) — 3GPP TS 29.122 V18.6.0 T8 reference point for Northbound APIs
- [TS29122_DeviceTriggering.yaml](TS29122_DeviceTriggering.yaml) — 3GPP TS 29.122 V18.6.0 T8 reference point for Northbound APIs
- [TS29122_ECRControl.yaml](TS29122_ECRControl.yaml) — 3GPP TS 29.122 V17.6.0 T8 reference point for Northbound APIs
- [TS29122_GMDviaMBMSbyMB2.yaml](TS29122_GMDviaMBMSbyMB2.yaml) — 3GPP TS 29.122 V18.6.0 T8 reference point for Northbound APIs
- [TS29122_GMDviaMBMSbyxMB.yaml](TS29122_GMDviaMBMSbyxMB.yaml) — 3GPP TS 29.122 V18.6.0 T8 reference point for Northbound APIs
- [TS29122_MonitoringEvent.yaml](TS29122_MonitoringEvent.yaml) — 3GPP TS 29.122 V18.6.0 T8 reference point for Northbound APIs
- [TS29122_MsisdnLessMoSms.yaml](TS29122_MsisdnLessMoSms.yaml) — 3GPP TS 29.122 V17.6.0 T8 reference point for Northbound APIs
- [TS29122_NIDD.yaml](TS29122_NIDD.yaml) — 3GPP TS 29.122 V18.6.0 T8 reference point for Northbound APIs
- [TS29122_NpConfiguration.yaml](TS29122_NpConfiguration.yaml) — 3GPP TS 29.122 V18.6.0 T8 reference point for Northbound APIs
- [TS29122_PfdManagement.yaml](TS29122_PfdManagement.yaml) — 3GPP TS 29.122 V18.6.0 T8 reference point for Northbound APIs
- [TS29122_RacsParameterProvisioning.yaml](TS29122_RacsParameterProvisioning.yaml) — 3GPP TS 29.122 V18.6.0 T8 reference point for Northbound APIs
- [TS29122_ReportingNetworkStatus.yaml](TS29122_ReportingNetworkStatus.yaml) — 3GPP TS 29.122 V18.6.0 T8 reference point for Northbound APIs
- [TS29122_ResourceManagementOfBdt.yaml](TS29122_ResourceManagementOfBdt.yaml) — 3GPP TS 29.122 V18.6.0 T8 reference point for Northbound APIs
- [TS29502_Nsmf_PDUSession.yaml](TS29502_Nsmf_PDUSession.yaml) — 3GPP TS 29.502 V18.11.0; 5G System; Session Management Services; Stage 3
- [TS29503_Nudm_EE.yaml](TS29503_Nudm_EE.yaml) — 3GPP TS 29.503 Unified Data Management Services, version 18.12.0
- [TS29503_Nudm_MT.yaml](TS29503_Nudm_MT.yaml) — 3GPP TS 29.503 Unified Data Management Services, version 18.6.0
- [TS29503_Nudm_NIDDAU.yaml](TS29503_Nudm_NIDDAU.yaml) — 3GPP TS 29.503 Unified Data Management Services, version 18.6.0
- [TS29503_Nudm_PP.yaml](TS29503_Nudm_PP.yaml) — 3GPP TS 29.503 Unified Data Management Services, version 18.12.0
- [TS29503_Nudm_RSDS.yaml](TS29503_Nudm_RSDS.yaml) — 3GPP TS 29.503 Unified Data Management Services, version 18.6.0
- [TS29503_Nudm_SDM.yaml](TS29503_Nudm_SDM.yaml) — 3GPP TS 29.503 Unified Data Management Services, version 18.13.0
- [TS29503_Nudm_SSAU.yaml](TS29503_Nudm_SSAU.yaml) — 3GPP TS 29.503 Unified Data Management Services, version 18.11.0
- [TS29503_Nudm_UEAU.yaml](TS29503_Nudm_UEAU.yaml) — 3GPP TS 29.503 Unified Data Management Services, version 18.9.0
- [TS29503_Nudm_UECM.yaml](TS29503_Nudm_UECM.yaml) — 3GPP TS 29.503 Unified Data Management Services, version 18.9.0
- [TS29503_Nudm_UEID.yaml](TS29503_Nudm_UEID.yaml) — 3GPP TS 29.503 Unified Data Management Services, version 17.10.0
- [TS29504_Nudr_DR.yaml](TS29504_Nudr_DR.yaml) — 3GPP TS 29.504 V18.13.0; 5G System; Unified Data Repository Services; Stage 3
- [TS29504_Nudr_GroupIDmap.yaml](TS29504_Nudr_GroupIDmap.yaml) — 3GPP TS 29.504 V18.12.0; 5G System; Unified Data Repository Services; Stage 3
- [TS29505_Subscription_Data.yaml](TS29505_Subscription_Data.yaml) — 3GPP TS 29.505 V18.7.0; 5G System; Usage of the Unified Data Repository Service for subscription data; Stage 3
- [TS29508_Nsmf_EventExposure.yaml](TS29508_Nsmf_EventExposure.yaml) — 3GPP TS 29.508 V18.9.0; 5G System; Session Management Event Exposure Service.
- [TS29510_Nnrf_AccessToken.yaml](TS29510_Nnrf_AccessToken.yaml) — 3GPP TS 29.510 V18.8.0; 5G System; Network Function Repository Services; Stage 3
- [TS29510_Nnrf_Bootstrapping.yaml](TS29510_Nnrf_Bootstrapping.yaml) — 3GPP TS 29.510 V18.7.0; 5G System; Network Function Repository Services; Stage 3
- [TS29510_Nnrf_NFDiscovery.yaml](TS29510_Nnrf_NFDiscovery.yaml) — 3GPP TS 29.510 V18.11.0; 5G System; Network Function Repository Services; Stage 3
- [TS29510_Nnrf_NFManagement.yaml](TS29510_Nnrf_NFManagement.yaml) — 3GPP TS 29.510 V18.11.0; 5G System; Network Function Repository Services; Stage 3
- [TS29518_Namf_Communication.yaml](TS29518_Namf_Communication.yaml) — 3GPP TS 29.518 V18.14.0; 5G System; Access and Mobility Management Services
- [TS29518_Namf_EventExposure.yaml](TS29518_Namf_EventExposure.yaml) — 3GPP TS 29.518 V18.10.0; 5G System; Access and Mobility Management Services
- [TS29518_Namf_Location.yaml](TS29518_Namf_Location.yaml) — 3GPP TS 29.518 V18.9.0; 5G System; Access and Mobility Management Services
- [TS29518_Namf_MBSBroadcast.yaml](TS29518_Namf_MBSBroadcast.yaml) — 3GPP TS 29.518 V18.6.0; 5G System; Access and Mobility Management Services
- [TS29518_Namf_MBSCommunication.yaml](TS29518_Namf_MBSCommunication.yaml) — 3GPP TS 29.518 V18.6.0; 5G System; Access and Mobility Management Services
- [TS29518_Namf_MT.yaml](TS29518_Namf_MT.yaml) — 3GPP TS 29.518 V18.6.0; 5G System; Access and Mobility Management Services
- [TS29520_Nnwdaf_AnalyticsInfo.yaml](TS29520_Nnwdaf_AnalyticsInfo.yaml) — 3GPP TS 29.520 V18.13.0; 5G System; Network Data Analytics Services.
- [TS29520_Nnwdaf_DataManagement.yaml](TS29520_Nnwdaf_DataManagement.yaml) — 3GPP TS 29.520 V18.13.0; 5G System; Network Data Analytics Services.
- [TS29520_Nnwdaf_EventsSubscription.yaml](TS29520_Nnwdaf_EventsSubscription.yaml) — 3GPP TS 29.520 V18.13.0; 5G System; Network Data Analytics Services.
- [TS29520_Nnwdaf_MLModelMonitor.yaml](TS29520_Nnwdaf_MLModelMonitor.yaml) — 3GPP TS 29.520 V18.13.0; 5G System; Network Data Analytics Services.
- [TS29520_Nnwdaf_MLModelProvision.yaml](TS29520_Nnwdaf_MLModelProvision.yaml) — 3GPP TS 29.520 V18.13.0; 5G System; Network Data Analytics Services.
- [TS29520_Nnwdaf_MLModelTraining.yaml](TS29520_Nnwdaf_MLModelTraining.yaml) — 3GPP TS 29.520 V18.14.0; 5G System; Network Data Analytics Services.
- [TS29520_Nnwdaf_RoamingAnalytics.yaml](TS29520_Nnwdaf_RoamingAnalytics.yaml) — 3GPP TS 29.520 V18.13.0; 5G System; Network Data Analytics Services.
- [TS29520_Nnwdaf_RoamingData.yaml](TS29520_Nnwdaf_RoamingData.yaml) — 3GPP TS 29.520 V18.13.0; 5G System; Network Data Analytics Services.
- [TS29523_Npcf_EventExposure.yaml](TS29523_Npcf_EventExposure.yaml) — 3GPP TS 29.523 V18.5.0; 5G System; Policy Control Event Exposure Service; Stage 3.
- [TS29554_Npcf_BDTPolicyControl.yaml](TS29554_Npcf_BDTPolicyControl.yaml) — 3GPP TS 29.554 V18.3.0; 5G System; Background Data Transfer Policy Control Service.

- [TS29564_Nupf_EventExposure.yaml](TS29564_Nupf_EventExposure.yaml) — 3GPP TS 29.564 V18.6.0; 5G System; User Plane Function Services; Stage 3.
- [TS29564_Nupf_GetUEPrivateIPaddrAndIdentifiers.yaml](TS29564_Nupf_GetUEPrivateIPaddrAndIdentifiers.yaml) — 3GPP TS 29.564 V18.5.0; 5G System; 5G System; User Plane Function Services; Stage 3
- [TS29571_CommonData.yaml](TS29571_CommonData.yaml) — 3GPP TS 29.571 Common Data Types for Service Based Interfaces, version 18.12.0
- [TS29574_Ndccf_ContextManagement.yaml](TS29574_Ndccf_ContextManagement.yaml) — 3GPP TS 29.574 V18.11.0; 5G System; Data Collection Coordination Services; Stage 3.
- [TS29574_Ndccf_DataManagement.yaml](TS29574_Ndccf_DataManagement.yaml) — 3GPP TS 29.574 V18.11.0; 5G System; Data Collection Coordination Services; Stage 3.
- [TS29575_Nadrf_DataManagement.yaml](TS29575_Nadrf_DataManagement.yaml) — 3GPP TS 29.575 V18.11.0; 5G System; Analytics Data Repository Services; Stage 3.
- [TS29575_Nadrf_MLModelManagement.yaml](TS29575_Nadrf_MLModelManagement.yaml) — 3GPP TS 29.575 V18.7.0; 5G System; Analytics Data Repository Services; Stage 3.
- [TS29576_Nmfaf_3caDataManagement.yaml](TS29576_Nmfaf_3caDataManagement.yaml) — 3GPP TS 29.576 V18.7.0; 5G System; Messaging Framework Adaptor Services; Stage 3.
- [TS29576_Nmfaf_3daDataManagement.yaml](TS29576_Nmfaf_3daDataManagement.yaml) — 3GPP TS 29.576 V18.4.0; 5G System; Messaging Framework Adaptor Services; Stage 3.
- [manifest.yaml](manifest.yaml)

## Unresolved external dependencies

- `TS29507_Npcf_AMPolicyControl.yaml`
- `TS29509_Nausf_SoRProtection.yaml`
- `TS29509_Nausf_UEAuthentication.yaml`
- `TS29509_Nausf_UPUProtection.yaml`
- `TS29512_Npcf_SMPolicyControl.yaml`
- `TS29514_Npcf_PolicyAuthorization.yaml`
- `TS29515_Ngmlc_Location.yaml`
- `TS29517_Naf_EventExposure.yaml`
- `TS29519_Application_Data.yaml`
- `TS29519_Exposure_Data.yaml`
- `TS29519_Policy_Data.yaml`
- `TS29522_AMPolicyAuthorization.yaml`
- `TS29522_ServiceParameter.yaml`
- `TS29522_TrafficInfluence.yaml`
- `TS29531_Nnssf_NSSelection.yaml`
- `TS29534_Npcf_AMPolicyAuthorization.yaml`
- `TS29536_Nnsacf_SliceEventExposure.yaml`
- `TS29562_Nhss_imsUEAU.yaml`
- `TS29565_Ntsctsf_QoSandTSCAssistance.yaml`
- `TS29572_Nlmf_Location.yaml`
- `TS29573_N32_Handshake.yaml`
- `TS29591_Nnef_EventExposure.yaml`
- `TS32291_Nchf_ConvergedCharging.yaml`
