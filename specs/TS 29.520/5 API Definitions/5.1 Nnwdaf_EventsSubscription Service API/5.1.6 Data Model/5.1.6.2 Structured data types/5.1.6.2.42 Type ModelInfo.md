---
spec: TS 29.520
version: 18.14.0
release: '18'
clause: 5.1.6.2.42
title: 5.1.6.2.42 Type ModelInfo
source_archive: 29520-ie0.zip
source_document: 29520-ie0.doc
source_archive_sha256: 4937d4ffcdc8760cac3dd990b10d5c0beb722f9eab26d94ce4b1f296287057c1
source_document_sha256: 9c9beb10bbac50abf869d1cc575a55184a8a97008a072a839b3da2ebe4aaeb69
content_origin: 3gpp-source
conversion: deterministic-pandoc-structure
---

# 5.1.6.2.42 Type ModelInfo


Table 5.1.6.2.42-1: Definition of type ModelInfo

|                |                    |     |             |                                                                                                          |               |
|----------------|--------------------|-----|-------------|----------------------------------------------------------------------------------------------------------|---------------|
| Attribute name | Data type          | P   | Cardinality | Description                                                                                              | Applicability |
| analyticsId    | NwdafEvent         | M   | 1           | Type of analytics for which this ML model is used.                                                       |               |
| mlModelInfos   | array(MLModelInfo) | M   | 1..N        | The information of the ML models which are applicable to the event indicated by "analyticsId" attribute. |               |
